...
1. **タスクURLの確認**: まずユーザーに「このPRに関連するタスクURL（issueやチケットなど）はありますか？」と質問し、回答を待つ。URLがあればPR本文に含める。なければそのまま進める。
2. @.github/PULL_REQUEST_TEMPLATE.md または @.github/pull_request_template.md のテンプレートを読み込む（存在する場合）
3. 現在のブランチの変更内容を分析（`git status`, `git diff`, `git log`）
4. 簡潔で要点を押さえた日本語のPR説明文を生成
5. `gh pr create --draft` でDraft PRを作成
